...
1. Use an existing method for differential abundance.
    ...
    $$\tilde{\theta_d} = \mathrm{mean}_{\mathrm{post}} \, f(Y) - \mathrm{mean}_{\mathrm{pre}} \, f(Y).$$
...
```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figures/data_sim.png")
```
...
```{r, echo = TRUE, message = FALSE,  tidy=TRUE, tidy.opts=list(width.cutoff=50)}
## Fitting and analyzing the original ALDEx2 model
mod.base <- aldex(Y, conds)
row.names(mod.base %>% filter(we.eBH < 0.05))
```
...
```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figures/default-scale.jpg")
```

## Decoding the Default Scale Model

```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figures/theta-est.jpg")
```
...
<p style="text-align: center;">**No taxa are significant!**</p>
...
- Two sample example:
`aldex.makeScaleMatrix(gamma <- 1, mu <- c(1,.9), conds <- c("pre", "post"), log = FALSE)`
...
row.names(mod.flow %>% filter(we.eBH < 0.05))
```
...
```{r, echo=FALSE, out.width = '50%', fig.align='center'}
knitr::include_graphics("figures/sen-analysis.jpg")
```
